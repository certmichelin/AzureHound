...
func listMembersOfGroupCmdImpl(cmd *cobra.Command, _ []string) {
	...
	start := time.Now()
	stream := listMembersOfGroup(ctx, azClient, groupID)
...
func listMembersOfGroup(ctx context.Context, client client.AzureClient, groupName string) <-chan interface{} {
	...
	go func() {
		...
		defer close(out)
		count := 0
		for item := range client.ListAzureADMembersOfGroup(ctx, params, groupName) {
			if item.Error != nil {
