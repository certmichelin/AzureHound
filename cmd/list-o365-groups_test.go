...
func TestListO365Groups(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	ctx := context.Background()

	mockClient := mocks.NewMockAzureClient(ctrl)
	mockChannel := make(chan client.AzureResult[azure.O365Group])
	mockTenant := azure.Tenant{}
	mockError := fmt.Errorf("I'm an error")
	mockClient.EXPECT().TenantInfo().Return(mockTenant).AnyTimes()
	mockClient.EXPECT().ListAzureADO365Groups(gomock.Any(), gomock.Any()).Return(mockChannel)

	go func() {
		defer close(mockChannel)
		mockChannel <- client.AzureResult[azure.O365Group]{
			Ok: azure.O365Group{},
		}
		mockChannel <- client.AzureResult[azure.O365Group]{
			Error: mockError,
	...
	channel := listO365Groups(ctx, mockClient)
	result := <-channel
